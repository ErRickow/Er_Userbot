...
# <https://www.github.com/Teameruserbot/eruserbot/blob/main/LICENSE/>.
...
from erNganu._misc import SUDO_M, owner_and_sudos
...
"""
#**eruserbot [v{eruserbot_version}](https://github.com/Teameruserbot/eruserbot), powered by @Teameruserbot**
...
async def own(event):
    msg = Owner_info_msg.format(
        mention=event.sender.mention, me=inline_mention(eruser_bot.me)
    )
    if custom_info:
        msg += "\n\n• Powered by **@chakszzz**"
    await event.edit(
...
async def eruserbot(event):
    ...
    if event.sender_id not in SUDO_M.fullsudos:
        ok = ""
        me = inline_mention(eruser_bot.me)
        ...
        if not udB.get_key("STARTMSG"):
            ...
            await event.reply(
                f"Hey there {mention}, this is eruserbot Assistant of {me}!\n\n{ok}",
...
async def botstat(event):
    ...
    msg = """Er Userbot Assistant - Stats
Total Users - {}""".format(
        ok,
    )
