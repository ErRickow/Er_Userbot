...
from erNganu.fns.misc import google_search
...
async def twitter_search(event):
    ...
    if _er_cache.get("twitter"):
        _er_cache["twitter"].update({match: reso})
    else:
        _er_cache.update({"twitter": {match: reso}})
