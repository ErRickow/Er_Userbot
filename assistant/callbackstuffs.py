...
try:
    from erNganu.fns.gDrive import GDriveManager
except ImportError:
    GDriveManager = None
...
from erNganu.fns.helper import fast_download, progress
from erNganu.fns.tools import Carbon, async_searcher, get_paste, telegraph_client
from erNganu.startup.loader import Loader
...
_buttons = {
    ...
    "alvcstm": {
        "text": f"Custom {HNDLR}alive. Pilih opsinya dibawah -",
        "buttons": [
    ...
    "chatbot": {
        "text": "Fitur ini bisa untuk chatbot.\n[More info](https://t.me/pamerdong)",
        "buttons": [
            [
...
_convo = {
    ...
    "pmlgg": {
        "var": "PMLOGGROUP",
        "name": "Pm Log Group",
        "text": "Kirim chat id group yang akan buat save log.",
        "back": "pml",
    },
    "vcs": {
        "var": "VC_SESSION",
    ...
    "settag": {
        "var": "TAG_LOG",
        "name": "Tag Log Group",
        "text": f"Buat group, tambahkan assisten ke dalam group dan jadikan admin.\nGunakan `{HNDLR}id` Untuk mendapatkan id group.\n\nPencet /cancel untuk tidak jadi.",
        "back": "taglog",
    },
    "alvtx": {
        "var": "ALIVE_TEXT",
        "name": "Alive Text",
        "text": "**Alive Text**\nMasukkan Alive teks nya.\n\nPencet /cancel untuk tidak jadi.",
        "back": "cbs_alvcstm",
    },
    "sfexf": {
        "var": "EXCLUDE_FED",
...
TOKEN_FILE = "nganu/auths/auth_token.txt"
...
async def send(eve):
    ...
    thumb = "nganu/gambar/cater.jpg"
    await eve.answer("■ Mengirim ■")
    data = f"uh_{key}_"
    index = None
    if "|" in name:
...
async def update(eve):
    ...
    if heroku_api:
        ...
        execl(sys.executable, sys.executable, "-m", "erNganu")
...
async def changes(okk):
    ...
    cli = "\n\nKlik button dibawah untuk update!"
    if not match:
        try:
...
async def eddon(event):
    ...
    await event.reply(
        "Selesai! Addons Telah di aktifkan!!\n\n Setelah Semua setting selesai, lakukan restart.",
...
async def eddof(event):
    ...
    await event.reply(
        "Selesai!Addons telah dimatikan!! Setelah Semua Setting Selesai, lakukan restart.",
        buttons=get_back_button("eaddon"),
    )
