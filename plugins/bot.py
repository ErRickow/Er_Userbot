...
async def _(event):
    ...
    file = f"eruserbot{sys.argv[-1]}.log" if len(sys.argv) > 1 else "eruserbot.log"
    if opt == "heroku":
        ...
        if isinstance(file, dict):
            ...
            return
        await event.reply("**Er Userbot Logs.**", file=file)
    elif opt == "open":
        with open("eruserbot.log", "r") as f:
            file = f.read()[-4000:]
