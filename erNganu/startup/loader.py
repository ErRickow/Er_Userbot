...
def _after_load(loader, module, plugin_name=""):
    ...
    if doc_ := get_help(plugin_name) or module.__doc__:
        try:
            ...
            loader._logger.info(f"Error di {plugin_name}: {module}")
            return
...
def load_other_plugins(addons=None, pmbot=None, manager=None, vcbot=None):
    ...
    if addons:
        ...
        if not os.path.exists("addons"):
            subprocess.run(
                f"git clone -q -b {Repo().active_branch} https://github.com/ErRickow/ErUbotAddons.git addons",
                shell=True,
            )
        else:
        ...
        if not os.path.exists("addons"):
            subprocess.run(
                "git clone -q https://github.com/ErRickow/ErUbotAddons.git addons",
                shell=True,
            )
