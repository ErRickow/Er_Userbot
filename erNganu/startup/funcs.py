...
async def autopilot():
    ...
    if not channel:
        ...
        try:
            ...
            LOGS.exception(er)
            LOGS.info(
                "Ada yang salah!! buat bot sendiri di @BotFather dan copy token bot sebagai vars BOT_TOKEN, dan restart."
            )
            ...
            async for x in eruser_bot.iter_messages(
                ...
                if not os.path.exists(plugin):
                    ...
                    try:
                        ...
                        LOGS.info(f"ErUserbot - PLUGIN_CHANNEL - ERROR - {plugin}")
                        LOGS.exception(e)
                        os.remove(plugin)
        except Exception as er:
            LOGS.exception(er)
...
async def WasItRestart(udb):
    ...
    try:
        ...
        await who.edit_message(
            int(data[1]), int(data[2]), "__Restart Success Sayang.__"
        )
    except Exception as er:
        LOGS.exception(er)
