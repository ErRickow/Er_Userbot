...
if run_as_module:
    ...
    from ..version import __version__ as __erNganu__
    from ..version import eruserbot_version

    file = f"eruserbot{sys.argv[6]}.log" if len(sys.argv) > 6 else "eruserbot.log"

    if os.path.exists(file):
        os.remove(file)

    HOSTED_ON = where_hosted()
    LOGS = getLogger("erUbotLogs")
    ...
    LOGS.info(f"er-Nganu Version - {__erNganu__}")
    LOGS.info(f"Telethon Version - {__version__} [Layer: {LAYER}]")
    LOGS.info(f"Er Userbot Version - {eruserbot_version} [{HOSTED_ON}]")
