...
async def _callback_check(event):
    ...
    if not _er_cache.get("admin_callback"):
        _er_cache.update({"admin_callback": {id_: None}})
    else:
        _er_cache["admin_callback"].update({id_: None})
    while not _er_cache["admin_callback"].get(id_):
        await asyncio.sleep(1)
    key = _er_cache.get("admin_callback", {}).get(id_)
    del _er_cache["admin_callback"][id_]
    return key


async def get_update_linked_chat(event):
    if _er_cache.get("LINKED_CHATS") and _er_cache["LINKED_CHATS"].get(event.chat_id):
        _ignore = _er_cache["LINKED_CHATS"][event.chat_id]["linked_chat"]
    else:
        channel = await event.client(
            functions.channels.GetFullChannelRequest(event.chat_id)
        )
        _ignore = channel.full_chat.linked_chat_id
        if _er_cache.get("LINKED_CHATS"):
            _er_cache["LINKED_CHATS"].update({event.chat_id: {"linked_chat": _ignore}})
        else:
            _er_cache.update(
...
async def admin_check(event, require=None, silent: bool = False):
    ...
    if not perms.is_admin:
        if not silent:
            await event.reply("Lu Siapa Kontol!!", time=8)
        return
