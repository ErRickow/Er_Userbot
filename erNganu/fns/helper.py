# erroid - UserBot
# Copyright (C) 2021-2023 Teamerroid
#
# This file is a part of < https://github.com/Teamerroid/erroid/ >
# PLease read the GNU Affero General Public License in
# <https://github.com/Teamerroid/erNganu/blob/main/LICENSE>.
...
if run_as_module:
    ...
    async def safeinstall(event):
        ...
        if not event.reply_to:
            return await eod(
                event, f"Gunakan `{HNDLR}install` reply ke file .py."
            )
        ok = await eor(event, "`sedang menginstall...`")
        ...
        if plug in list(LOADED):
            return await eod(ok, f"Plugin `{plug}` sudah terinstall.")
    ...
    async def heroku_logs(event):
        ...
        ok = app.get_log()
        with open("eruserbot-heroku.log", "w") as log:
            log.write(ok)
        ...
        os.remove("ErUserbot-heroku.log")
        await xx.delete()

    async def def_logs(er, file):
        await er.respond(
            "**Er Userbot Logs.**",
            file=file,
            thumb=erConfig.thumb,
    ...
    def gen_chlog(repo, diff):
        ...
        ch_log = tldr_log = ""
        ch = f"<b>Er Userbot {eruserbot_version} updates for <a href={UPSTREAM_REPO_URL}/tree/{ac_br}>[{ac_br}]</a>:</b>"
        ch_tl = f"Er Userbot {eruserbot_version} updates for {ac_br}:"
        d_form = "%d/%m/%y || %H:%M"
...
async def restart(er=None):
    if Var.HEROKU_APP_NAME and Var.HEROKU_API:
        try:
            Heroku = heroku3.from_key(Var.HEROKU_API)
            app = Heroku.apps()[Var.HEROKU_APP_NAME]
            if er:
                await er.edit("`Restarting your app, please wait for a minute!`")
            app.restart()
...
async def shutdown(er):
    from .. import HOSTED_ON, LOGS

    er = await eor(er, "_Mematikan Er Userbot_")
    if HOSTED_ON == "heroku":
        if not (Var.HEROKU_APP_NAME and Var.HEROKU_API):
            return await er.edit("Please Fill `HEROKU_APP_NAME` and `HEROKU_API`")
        dynotype = os.getenv("DYNO").split(".")[0]
        try:
            Heroku = heroku3.from_key(Var.HEROKU_API)
            app = Heroku.apps()[Var.HEROKU_APP_NAME]
            await er.edit("`Shutting Down your app, please wait for a minute!`")
            app.process_formation()[dynotype].scale(0)
        except BaseException as e:
            LOGS.exception(e)
            return await er.edit(
                "`HEROKU_API` and `HEROKU_APP_NAME` is wrong! Kindly re-check in config vars."
            )
    else:
        sys.exit()
